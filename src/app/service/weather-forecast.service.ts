...
export class WeatherForecastService {
  ...
  getForecast(
      ...
      state:string,
      city:string) {
        const api = `http://localhost:3000/v1/forecast/${city}/${state}/${country}`;
  ...
  getCities(uf: string) {
    const state =  STATE_CITIES.find(state => state.sigla === uf);

    if (!state) return []

    return state.cidades.map(cidade => {return {label: cidade, value: cidade}});
  }
